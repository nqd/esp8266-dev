...
SPIFFS_buffer_bytes_for_filedescs(spiffs *fs, u32_t num_descs) {
  return num_descs * sizeof(spiffs_fd);
}
#if SPIFFS_CACHE
u32_t ICACHE_FLASH_ATTR
SPIFFS_buffer_bytes_for_cache(spiffs *fs, u32_t num_pages) {
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work,
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_errno(spiffs *fs) {
  return fs->err_code;
}

void ICACHE_FLASH_ATTR
SPIFFS_clearerr(spiffs *fs) {
  fs->err_code = SPIFFS_OK;
}

s32_t ICACHE_FLASH_ATTR
SPIFFS_creat(spiffs *fs, char *path, spiffs_mode mode) {
  (void)mode;
  SPIFFS_API_CHECK_CFG(fs);
...
spiffs_file ICACHE_FLASH_ATTR
SPIFFS_open(spiffs *fs, char *path, spiffs_flags flags, spiffs_mode mode) {
  (void)mode;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
spiffs_file ICACHE_FLASH_ATTR
SPIFFS_open_by_dirent(spiffs *fs, struct spiffs_dirent *e, spiffs_flags flags, spiffs_mode mode) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
spiffs_hydro_write(spiffs *fs, spiffs_fd *fd, void *buf, u32_t offset, s32_t len) {
  (void)fs;
  s32_t res = SPIFFS_OK;
  s32_t remaining = len;
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_remove(spiffs *fs, char *path) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_fremove(spiffs *fs, spiffs_file fh) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
static s32_t ICACHE_FLASH_ATTR
spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat *s) {
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_stat(spiffs *fs, char *path, spiffs_stat *s) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_fstat(spiffs *fs, spiffs_file fh, spiffs_stat *s) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
static s32_t ICACHE_FLASH_ATTR
spiffs_fflush_cache(spiffs *fs, spiffs_file fh) {
  s32_t res = SPIFFS_OK;
#if SPIFFS_CACHE_WR
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_fflush(spiffs *fs, spiffs_file fh) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  s32_t res = SPIFFS_OK;
#if SPIFFS_CACHE_WR
...
void ICACHE_FLASH_ATTR
SPIFFS_close(spiffs *fs, spiffs_file fh) {
...
spiffs_DIR* ICACHE_FLASH_ATTR
SPIFFS_opendir(spiffs *fs, char *name, spiffs_DIR *d) {
  (void)name;
...
struct spiffs_dirent* ICACHE_FLASH_ATTR
SPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e) {
...
s32_t ICACHE_FLASH_ATTR
SPIFFS_info(spiffs *fs, u32_t *total, u32_t *used) {
  s32_t res = SPIFFS_OK;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
